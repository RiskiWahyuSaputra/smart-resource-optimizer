...
export default function DashboardPage() {
  ...
  const [storePreview, setStorePreview] = useState<DocumentPreview | null>(
    null,
  );
  ...
  const [incomingClaimActionId, setIncomingClaimActionId] = useState<
    number | null
  >(null);
      ...
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-3 px-6 text-center">
          ...
          <p className="text-sm text-slate-500">
            Anda sedang diarahkan kembali ke halaman login.
          </p>
        </div>
      </div>
    );
  ...
  const currentStatus = (user.profile?.verification_status ??
  ...
  const analyticsPeak = Math.max(
    ...analyticsSeries.map((point) => point.value),
    1,
  );
  ...
  const activeRestaurantPosts = myFoodPosts.filter(
    (foodPost) =>
      foodPost.status === "available" || foodPost.status === "claimed",
  );
  const expiringSoonFoodPosts = activeRestaurantPosts.filter((foodPost) => {
    const diffInMs =
      new Date(foodPost.available_until).getTime() - overviewTimestamp;
  ...
  const completedCommunityClaims = myClaims.filter(
  ...
  const verificationApprovalRate =
    totalVerificationRequests === 0
      ? 0
      : Math.round(
          (verificationCounts.verified / totalVerificationRequests) * 100,
        );
  ...
  const handleOpenDocument = async (verificationUser: VerificationUser) => {
    try {
      ...
      setDocumentPreview((currentPreview) => {
        ...
        return {
          fileName:
            verificationUser.profile?.document_url?.split("/").pop() ||
            "dokumen-verifikasi",
          url,
      ...
      setVerificationError(
        "Dokumen gagal dibuka. Pastikan file sudah tersedia.",
      );
    } finally {
  ...
  const handleCreateFoodPost = async (
    ...
    try {
      ...
      if (foodPostForm.description)
        formData.append("description", foodPostForm.description);
      ...
      formData.append(
        "available_until",
        new Date(foodPostForm.available_until).toISOString(),
      );
      ...
      setFoodPostFeedback(
        "Food post berhasil dibuat dan sudah tampil di marketplace.",
      );
  ...
  const handleUpdateFoodPostStatus = async (
    ...
    try {
      ...
      setFoodPostFeedback(
        `Status food post berhasil diubah menjadi ${formatFoodPostStatus(status)}.`,
      );
    } catch {
          ...
          {activeTab === "overview" && (
            <>
              ...
              <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
                {isAdmin ? (
                  <>
                    <button
                      onClick={() => setActiveTab("verification")}
                      className="flex flex-col items-center gap-2 rounded-2xl border border-slate-200 bg-white p-4 text-sm font-medium text-slate-700 shadow-sm transition-all hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 hover:-translate-y-1 hover:shadow-md"
              ...
              {!isAdmin && currentStatus !== "verified" && (
                <div
                  className={`mb-8 rounded-2xl border p-6 ${currentStatusConfig.wrapper}`}
                >
                  ...
                  <div className="grid w-full max-w-xl gap-3 sm:grid-cols-2">
                    {analyticsHighlights.length > 0 ? (
                      ...
                      <div className="rounded-2xl border border-dashed border-slate-200 bg-white/70 p-4 text-sm text-slate-500 sm:col-span-2">
                        Insight dampak akan tampil otomatis setelah aktivitas
                        mulai tercatat.
                      </div>
                    )}
                  </div>
                        ...
                        >
                          ...
                          <div className="relative z-10">
                            ...
                            <div className="mt-5">
                              ...
                              <p
                                className={`mt-3 text-sm font-medium transition-colors duration-300 ${toneClasses.text}`}
                              >
                                Ringkasan otomatis dari aktivitas{" "}
                                {isAdmin ? "platform" : "Anda"} saat ini.
                              </p>
                            </div>
                          </div>
                        </article>
                    ...
                    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                          ...
                          <h3 className="mt-2 text-xl font-bold text-slate-950">
                            {dashboardAnalytics?.chart.title ??
                              "Aktivitas 6 bulan terakhir"}
                          </h3>
                          <p className="mt-2 text-sm leading-6 text-slate-500">
                            Pola aktivitas utama ditampilkan ringkas agar mudah
                            dipantau dari ponsel sampai layar desktop.
                          </p>
                        </div>
                        <button
                          onClick={() => void loadDashboardOverviewAnalytics()}
                        ...
                        <div className="flex min-w-[340px] items-end gap-3 sm:gap-4">
                          {analyticsSeries.length > 0 ? (
                            ...
                            <div className="flex min-h-[240px] w-full items-center justify-center rounded-[24px] border border-dashed border-slate-200 bg-slate-50 px-6 text-center text-sm text-slate-500">
                              Grafik akan aktif setelah ada histori data yang
                              cukup untuk divisualisasikan.
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="mt-6 flex flex-wrap items-center gap-3 text-sm text-slate-500">
                        <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 font-medium text-emerald-700">
                          ...
                          {dashboardAnalytics?.chart.series_label ??
                            "Aktivitas"}
                        </span>
                          ...
                          <>
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                              <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700">
                                Pending:{" "}
                                {formatNumber(verificationCounts.pending)}
                              </span>
                              <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">
                                Verified:{" "}
                                {formatNumber(verificationCounts.verified)}
                              </span>
                              <span className="rounded-full bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-700">
                                Rejected:{" "}
                                {formatNumber(verificationCounts.rejected)}
                              </span>
                            </div>

                            <div className="mt-6 space-y-4">
                              {pendingVerificationUsers.length > 0 ? (
                                pendingVerificationUsers.map(
                                  (verificationUser) => (
                                    <article
                        ...
                        <>
                          ...
                          <div className="mt-6 space-y-4">
                            {analyticsHighlights.length > 0 ? (
                              ...
                              <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-sm text-slate-500">
                                Insight tambahan akan muncul setelah Anda mulai
                                beraktivitas.
                              </div>
                            )}
                          </div>
                    ...
                    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                          ...
                          <p className="mt-2 text-sm leading-6 text-slate-500">
                            Metrik kecil ini membantu Anda membaca kondisi
                            dashboard tanpa harus pindah tab.
                          </p>
                        </div>
                      </div>
                        ...
                        {overviewSnapshotItems.map((item, index) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="mt-4 text-sm leading-6 text-slate-500 transition-colors duration-300 group-hover:text-slate-600">
                                {item.caption}
                              </p>
                            </article>
                          );
                      ...
                      <div className="mt-6 overflow-hidden rounded-[28px] bg-slate-950 p-6 text-white">
                        ...
                        <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                          ...
                          {overviewFocus.mode === "link" ? (
                            ...
                            <button
                              onClick={() =>
                                setActiveTab(overviewFocus.targetTab)
                              }
                              className="inline-flex items-center justify-center rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-emerald-50"
                            >
                              {overviewFocus.actionLabel}
                        ...
                        {overviewRecentItems.length > 0 ? (
                          overviewRecentItems.map((item, index) => (
                            ...
                            >
                              <div className="flex items-start justify-between gap-4">
                                <div className="flex-1">
                                  <p className="font-semibold text-slate-950 transition-colors duration-300 group-hover:text-emerald-700">
                                    {item.title}
                                  </p>
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <p className="text-slate-500">
                    Buka dokumen, cek detail akun, lalu setujui atau tolak
                    pengajuan.
                  </p>
                </div>

                <div className="flex flex-wrap gap-2">
                  {(Object.keys(filterLabels) as VerificationFilter[]).map(
                    (filter) => (
                      <button
                        key={filter}
                ...
                <div className="grid gap-4">
                  {verificationUsers.map((verificationUser) => {
                    const verificationStatus =
                      verificationUser.profile?.verification_status ??
                      "pending";
                      ...
                      >
                        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                          ...
                          <div className="flex w-full max-w-sm flex-col gap-3">
                            <button
                              onClick={() =>
                                void handleOpenDocument(verificationUser)
                              }
                              disabled={
                            ...
                            {verificationUser.role === "restaurant" && (
                              <button
                                onClick={() =>
                                  handleOpenStoreImage(verificationUser)
                                }
                                disabled={
              ...
              {currentStatus !== "verified" && (
                <div className="mb-6 rounded-2xl border border-amber-100 bg-amber-50 p-5 text-amber-800">
                  ...
                  <p className="mt-1 text-sm">
                    Akun restoran Anda masih menunggu verifikasi. Setelah status
                    berubah menjadi verified, Anda bisa menambah dan
                    mengaktifkan stok makanan di marketplace.
                  </p>
                </div>
              )}
                ...
                >
                  <div className="mb-6 flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        Informasi ini akan langsung tampil di marketplace
                        setelah disimpan.
                      </p>
                    </div>
                  ...
                  <div className="grid gap-5 md:grid-cols-2">
                    <div className="md:col-span-2">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          handleFoodPostFormChange("title", event.target.value)
                        }
                    ...
                    <div className="md:col-span-2">
                      ...
                      <LocationPicker
                        initialLat={
                          foodPostForm.lat
                            ? Number(foodPostForm.lat)
                            : undefined
                        }
                        initialLng={
                          foodPostForm.long
                            ? Number(foodPostForm.long)
                            : undefined
                        }
                        onChange={(lat, lng) => {
              ...
              {foodPostsLoading ? (
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
                  ...
                  <p className="mx-auto mb-6 max-w-xs text-sm text-slate-500">
                    Mulai bagikan kelebihan makanan Anda untuk membantu
                    ...
                    >
                      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                        <div className="space-y-4">
                          <div>
                            ...
                            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                              {foodPost.description ||
                                "Belum ada deskripsi tambahan untuk postingan ini."}
                            </p>
                          </div>
                          ...
                          <div className="grid gap-3 sm:grid-cols-3">
                            ...
                            <div className="rounded-2xl bg-slate-50 px-4 py-3">
                              ...
                              <p className="mt-1 text-sm text-slate-700">
                                {formatDate(foodPost.created_at)}
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex w-full max-w-sm flex-col gap-3">
                          <button
                            onClick={() =>
                              void handleUpdateFoodPostStatus(
                                foodPost.id,
                                "available",
                              )
                            }
                            disabled={
                              foodPostActionId === foodPost.id ||
                              foodPost.status === "available"
                            }
            ...
            <div>
              <div className="mb-8 flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-slate-500">
                    Tinjau permintaan komunitas yang ingin mengambil food post
                    Anda.
                  </p>
                </div>
                <div className="rounded-full bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700">
                  Pending:{" "}
                  {
                    incomingClaims.filter((claim) => claim.status === "pending")
                      .length
                  }
                </div>
              </div>
              ...
              {incomingClaimsLoading ? (
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
                  ...
                  <p className="mx-auto max-w-xs text-sm text-slate-500">
                    Saat komunitas mengklaim stok makanan Anda dari marketplace,
                    datanya akan muncul di sini.
                  </p>
                </div>
              ) : (
                <div className="grid gap-4">
                      ...
                      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                        <div className="space-y-4">
                          <div>
                            ...
                            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                              {claim.notes ||
                                "Belum ada catatan tambahan dari komunitas."}
                            </p>
                          </div>
                        ...
                        <div className="flex w-full max-w-sm flex-col gap-3">
                          <button
                            onClick={() =>
                              void handleIncomingClaimStatus(
                                claim.id,
                                "approved",
                              )
                            }
                            disabled={
                              incomingClaimActionId === claim.id ||
          ...
          {activeTab === "claims" && !isRestaurant && !isAdmin && (
            <div>
              <div className="mb-8">
                ...
                <p className="text-slate-500">
                  Daftar makanan yang sedang Anda klaim.
                </p>
              </div>
              ...
              {claimsLoading ? (
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
                  ...
                  <p className="mx-auto mb-6 max-w-xs text-sm text-slate-500">
                    Jelajahi marketplace untuk menemukan makanan gratis yang
                      ...
                      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                        <div className="space-y-4">
                          <div>
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                              {claim.food_post.user.profile?.name ||
                                claim.food_post.user.name}
                            </p>
                            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                              {claim.notes ||
                                "Belum ada catatan tambahan untuk klaim ini."}
                            </p>
                          </div>
                          ...
                          <div className="grid gap-3 sm:grid-cols-3">
                            <div className="rounded-2xl bg-slate-50 px-4 py-3">
                              ...
                              {claim.food_post.lat && claim.food_post.long && (
                                <a
                                  href={`https://www.google.com/maps/dir/?api=1&destination=${claim.food_post.lat},${claim.food_post.long}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="mt-2 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-600 !text-white text-xs font-bold rounded-lg hover:bg-blue-700 transition-colors shadow-sm w-full"
                                >
                                  <span>🗺️</span>
                                  Buka Navigasi
                                </a>
                            ...
                            <div className="rounded-2xl bg-slate-50 px-4 py-3">
                              ...
                              <p className="mt-1 text-sm text-slate-700">
                                {formatFoodPostExpiry(
                                  claim.food_post.available_until,
                                )}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-slate-50 p-4">
                          ...
                          <div className="mt-4">
                            <button
                              onClick={() =>
                                void handleCommunityClaimStatus(
                                  claim.id,
                                  "cancelled",
                                )
                              }
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
            <div className="flex h-[85vh] w-full max-w-5xl flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-2xl">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 px-6 py-4">
                <p className="text-sm text-slate-500">
                  Dokumen ditampilkan dalam popup agar proses verifikasi tetap
                  di halaman ini.
                </p>
        ...
        {storePreview && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
            <div className="flex h-[85vh] w-full max-w-5xl flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-2xl">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 px-6 py-4">
                <p className="text-sm text-slate-500">
                  Gunakan foto ini untuk memverifikasi keaslian lokasi fisik
                  restoran/toko.
                </p>
                <div className="flex items-center gap-3">
                  <a
